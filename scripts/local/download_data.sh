...
for machine in "${available_machine_types[@]}"; do
  ...
  for eval_train in ${download_dir}/eval/${machine}/train/*.wav; do
    cp "${eval_train}" "${download_dir}/dev/${machine}/train"
  done
done

if [ ! -d "${download_dir}/dcase2022_evaluator" ]; then
  cd "${download_dir}"
  git clone https://github.com/Kota-Dohi/dcase2022_evaluator.git
  log "Successfully downloaded ground truth dataset."
else
  log "Successfully ground truth dataset was already downloaded."
fi
